...
- **Codecs.** Baseline JPEG decodes through Pillow, and RLE through pydicom's own decoder. JPEG Lossless, JPEG-LS, JPEG 2000 and High-Throughput JPEG 2000 decode through pydicom's plugins where one is installed, and otherwise through `imagecodecs`; [installation](docs/installation.md#decode-limits) lists what a decode cannot catch. Strict validation on the way out.
