...
- **Modify a source file.** Ingest reads; anonymize and redact change an in-memory graph. Until `export()`, the session writes only the store, `isocenter.log` and files you ask for (a configuration, a key, a report or a cohort table). `export()` writes copies to a directory you name, so a crashed or abandoned run leaves the originals as they were. The store (`<name>.db` and `<name>_pixels.bin`) holds the original identifiers and pixels, so keep it where PHI may live.
