...
7. **A finding raised under your policy was not acted on.** A patient, study or instance whose last PHI scan (`audit()`, or the scan `export(check_burned_in=True)` runs) found a value that a rule of the policy that scan ran with acts on, and which no `anonymize()` pass since has acted on. "Acted on" means the value was replaced, shifted or removed, or was already what the rule asks. A remediation that declined did not act, so its entity reads `IDENTIFIED` after the pass and counts here as well as under condition 5, unless it was edited after its scan and the pass changed nothing else on it, which leaves it `UNSCANNED`. This is the entity reading `IDENTIFIED` in `session.phi_status_summary()`, counted over the whole store; section 5's line gives the count per level. Series are never scanned and never counted. An entity edited after its scan reads `UNSCANNED`, and does not grade under this condition; a patient, study or series edited after its scan grades under condition 8.
8. **A patient, study or series was edited after its PHI status was recorded.** A field the export writes from it into every file beneath it, or the scan reads on it, was assigned a new value after a scan or a pass recorded its status: `patient.patient_name`, `patient.patient_id`, `study.study_date`, `study.study_time`, `study.study_instance_uid`, `series.series_instance_uid`, `series.modality`, `series.series_number` or `series.equipment`. Such an entity reads `UNSCANNED`, and no scan has read the value the export will write, so it grades until one does: `audit()` reads it, and the line goes. An entity never scanned is not counted here -- it is the absence of a measurement, as above -- and neither is one whose field a pass wrote, which records its status after the write. Assigning the value a field already holds is not an edit. Section 5's line gives the count per level ([#767](https://github.com/kvnlng/Isocenter/issues/767)).
...
!!! note "A `WARNING` row needs a person; a correction is not reported"
    ...
    Four grade reasons have no row anywhere else, so the Grade Basis is the only place they appear: **an empty audit trail** (a clean ingest followed by `audit()` alone writes no row, and grades `REVIEW_REQUIRED` because nothing the run did is attested), **a verb with no evidence** (`anonymize()` or `redact()` did work and none of the rows it writes reached the audit log), **findings nobody acted on** (entities the last PHI scan left `IDENTIFIED`; condition 7 under [How the grade is decided](#how-the-grade-is-decided)), and **owners edited after their scan** (condition 8).
