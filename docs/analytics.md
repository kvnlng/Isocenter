...
8. **An entity was edited after its PHI status was recorded.** Its content was changed after a scan or a pass recorded its status, and no scan has read the change since. For an instance that is any change to it -- an attribute, a sequence, the pixels -- or to an item nested in it, at any depth, and any change to a field of its series the export writes into it: `series.series_instance_uid`, `series.modality`, `series.series_number` or `series.equipment`. For a patient or a study it is an assignment of a field the export writes from it into every file beneath it, or the scan reads on it: `patient.patient_name`, `patient.patient_id`, `study.study_date`, `study.study_time` or `study.study_instance_uid`. Such an entity reads `UNSCANNED`, and it grades until a scan reads it: `audit()` reads it, and the line goes. An entity never scanned is not counted here -- it is the absence of a measurement, as above. Neither is what the library writes itself after a scan: a pass records its status after what it writes, redaction carries the status it found ([#486](https://github.com/kvnlng/Isocenter/issues/486)), and so does the reversible lock (`lock_identities`), whose token is not PHI; each carries it only when it still applied, so an edit made before them is not hidden by them. Assigning the value a field already holds is not an edit. A nested item's own status is not counted. Section 5's line gives the count per level ([#767](https://github.com/kvnlng/Isocenter/issues/767)).
