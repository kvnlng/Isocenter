...
`lock_identities(report)` locks the patients the report's findings name, so the report `audit()` returns locks every patient its scan found. A list of Patient IDs works too, and may mix IDs and findings.
