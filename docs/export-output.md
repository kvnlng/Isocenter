...
`session.export(folder)` writes de-identified copies of the session's instances to a new directory. Until `export()`, the session writes only the store, `isocenter.log` and files you ask for (a configuration, a key, a report or a cohort table). This page describes the files: where they land, how they are encoded, what the export changes on the way out, and what the two checking options add.
