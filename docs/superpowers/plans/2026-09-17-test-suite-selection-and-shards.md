...
28. **Task 14 measurements**, from the first real map (3.14.7t, `1b4f2a77` = `417b761d`):
    ...
    - **Step 2, rule 2's breadth, per worker** (files out of 330, with the module's `TARGETS` row for comparison):
      ...
      `ingest_worker` exceeds a third, and exceeds its row, even after the per-worker refinement. `_ingest_results` is its only dispatcher, and every ingest in the suite goes through it, so no per-worker keying can narrow it further. Only a call-site-level trace could. That is recorded, not built. A worker edit now selects fewer files than the full suite~~, but not fewer than its row~~. **Corrected, item 32:** the files rule 2 selects are not a superset of the row. `ingest_worker` drops 46 of its row's 127 files by design, and `_worker_init` dropped 3 of its 13 until item 32 gave it the row. **Row fallbacks per dispatcher on this map: 0.** All six dispatching functions have records, so rule 2 never falls to a row for any of the five workers, or for the helper union.
...
32. **The review of #734 (CHANGES REQUESTED at `21e8ce5c`), all seven findings taken; rebased onto `cdd96a1b` (#732).** Spec §10 item 18 carries the design side.
    - **(1) Dispatchers by the call.** `pool_calls` yields every `run_parallel(...)` call, every pool method called on an attribute (`submit`, `map`, `imap`, `imap_unordered`, `starmap`, `starmap_async`, `map_async`, `apply_async`) and every pool maker (`Pool`, `ProcessPoolExecutor`, `ThreadPoolExecutor`). The handed function's last dotted part is taken from the first positional argument or from `func`/`fn`/`worker`. `dispatchers()` keys on that part, and rule 2 matches an edit's qualname on its last part. Pool makers, and hand-offs of an unnamed callable, are keyed `None`, which only a helper's union asks.
      - On the rebased tree the redaction hand-off sits in `DicomSession._apply_redaction_rules`.
      - `test_every_pool_call_in_the_package_resolves_to_a_dispatcher` finds the calls independently of the finder, and fails with the old finder (mutant M1).
      - **The review's real mutant** (`"pixel_loader": None` in `execute_redaction_task`): `pytest --changed --changed-base=HEAD` selected 455 tests, among them the three the review named. Run on 3.12 over every test file that mentions redaction (113 files, two runs), it was killed by 24 tests, and all 24 are in the selection.
      - **Helpers without a record of their own also take their row**, because the union of dispatchers missed 3 of `parallel.py`'s 13 files for `_worker_init`. This is the 5 worker-only functions, not the review's rejected blanket rule.
      - Over all 493 worker-run functions: 0 blind-dispatcher row fallbacks.
    - **(2) and (7) Glob readers.** Every changed path also selects the test files that call `glob`/`rglob` and hold a `*.ext` literal matching its basename (`glob_readers`).
      - For a `*.md` that is 6 files, about 3 s by the timings file; a broken anchor in `docs/performance.md` now selects `test_doc_anchors.py`, which fails.
      - For a `*.py` it is 12 files, among them `test_source_citations.py` and `test_documented_env_vars.py`, the two tests the review found in no row.
      - The live-source checks moved from `tests/test_changed_code_selects_its_tests.py` into `tests/test_the_selector_reads_the_live_source.py` (5.7 s). They read `isocenter/**/*.py` by glob, so every package edit selects their file, and the first file costs about 105 s. A live test fails if that file becomes a `*.py` reader again.
      - Residual: the detector sees only a literal in the test file itself, not a pattern built at run time or passed through a helper.
    - **(3)** A changed test file also selects the test files that name its stem.
    - **(4)** `load()` returns `(map, None)` or `(None, why)`. A map that does not parse, or lacks one of `sha`, `python`, `functions`, `workers`, `unmapped`, is no map, and the fallback line carries the reason. It was an INTERNALERROR.
