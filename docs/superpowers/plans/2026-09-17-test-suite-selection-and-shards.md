...
**Deviations at implementation** (measured, and recorded here so the record says what was actually built). Part 1, PR 1, 2026-09-21; spec §10 item 14 carries the same list:
...
9. **Not changed, recorded:** a test that launches `python -c`/`-m` importing `isocenter` without `cwd=` (five files: `test_an_unusable_key_file_is_not_cached.py`, `test_codecs_strict.py`, `test_ctp_integration.py`, `test_no_global_warning_filter.py`, `test_sidecar_gate_crosses_processes.py`) used to find the tree under test through the root on the child's `sys.path`; its child now finds `isocenter` through `PYTHONPATH` or the editable install. In the main checkout that is the same tree; in a worktree it is the same tree only with CLAUDE.md's `PYTHONPATH` recipe, which every run here used.
