...
- **A cohort summary**: how many patients and instances the session holds and, when the report is generated in the same session as a DICOM export, how many instances that export wrote of those it was asked for. A per-instance manifest can be written as a separate document.
...
The team writes a configuration file that names the de-identification profile, the tags to remove, replace, or date-shift, and the pixel regions to redact on each make and model of equipment. Isocenter does not start from nothing. With no configuration, or a file that names no profile, a [floor policy](configuration.md#the-floor-policy) applies: the 646 tag rules of the PS3.15 Annex E Basic Profile table (2026c), three of them changed to research defaults (Study Date shifted rather than emptied, Patient's Sex and Age kept). Private tags are removed unless the file says otherwise. A file removes less than that floor only by saying so explicitly. The configuration plus that floor is what a reviewer reads to see what will happen to the data; a rule Isocenter cannot honour -- an emptied Patient ID, a date shift on a tag that is not a date, a replacement the tag's value representation cannot hold -- makes the configuration fail to load rather than do something other than it says.

Three options bear on review:

- **Date shifting** is deterministic per patient within a project, so intervals between a patient's studies survive. The offset is derived from a secret held in the team's working store and is not recoverable from the de-identified files alone. It is not a guarantee that absolute dates cannot be recovered: anyone holding that store, or any copy of it, can recover exact dates, so it should not leave the team, and the secret cannot be written out of it or carried to another store ([What to keep](configuration.md#what-to-keep)); a date a whole-day shift moves keeps its weekday, and the configured range bounds the guess. Files exported by releases before 0.9.7 do not have this protection (advisory GHSA-phg9-vcvc-j4r7; see [Upgrading from 0.9.x](migration.md)).
- **UIDs are replaced, within stated limits.** Study, Series, SOP Instance and the other UIDs the table codes `U` are replaced by UIDs derived from the store's project secret, so references between exported files still resolve and the export does not carry the source UIDs. A UID in a private tag is not replaced ([#765](https://github.com/kvnlng/Isocenter/issues/765)), a UID written into free text is not found, and a reference to a redacted instance does not follow it to its new UID.
...
Some equipment writes patient identifiers into the image pixels themselves. Isocenter redacts rectangular regions of the pixels for each make and model the team configures, and can use optical character recognition on a sample of one machine's images to find where text actually lands before the team writes those regions. Images the source marks as carrying burned-in annotations (Burned In Annotation `YES`), and that no redaction has cleared, are still exported; the report lists each one and grades the run `REVIEW_REQUIRED`, so a person looks at them. Text in a region no one configured is not detected by the export or the grade: optical character recognition runs only when the team asks for it ([Burned-in text](ocr.md)).
...
Bug reports and questions about documented behaviour go to [GitHub Issues](https://github.com/kvnlng/Isocenter/issues), where they are answered in public and for free. Planned work is tracked in [milestones](https://github.com/kvnlng/Isocenter/milestones), and longer-term ideas are [open issues with no milestone](https://github.com/kvnlng/Isocenter/issues?q=is%3Aissue+is%3Aopen+no%3Amilestone). Contributions are welcome; [`good first issue`](https://github.com/kvnlng/Isocenter/labels/good%20first%20issue) and [`help wanted`](https://github.com/kvnlng/Isocenter/labels/help%20wanted) are good places to start, and [Contributing](developer_guide.md) covers the development setup.
