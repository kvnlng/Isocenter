...
def test_session_persistence(tmp_path, dummy_patient):
    ...
    # 1. Create Session
    with DicomSession(str(db_file)) as session:
        session.store.patients.append(dummy_patient)
        session.save()
        session.persistence_manager.shutdown()

        # 2. Reload Session -- nested, not sequenced: the first session is
        # still open when the second reads, which is the arrangement this
        # asserted before #371 closed either of them.
        with DicomSession(str(db_file)) as session2:
...
def test_load_config(tmp_path, config_file):
    with DicomSession(str(tmp_path / "dummy.db")) as session:
...
def test_load_empty_config(tmp_path):
    ...
    with DicomSession(str(tmp_path / "empty.db")) as session:
...
def test_execute_config_integration(tmp_path, dummy_patient, config_file):
    ...
    with DicomSession(db_file) as session:
        session.store.patients.append(dummy_patient)
